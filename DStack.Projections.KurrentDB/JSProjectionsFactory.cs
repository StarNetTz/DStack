...
namespace DStack.Projections.KurrentDB.Utils;
...
{
   KurrentDBProjectionManagementClient Cli;

    public Dictionary<string, string> Projections { get; set; }

    public JSProjectionsFactory(IConfiguration conf)
    {
        var settings = KurrentDBClientSettings.Create(conf["KurrentDB:ConnectionString"]);
        Cli = new KurrentDBProjectionManagementClient(settings);
